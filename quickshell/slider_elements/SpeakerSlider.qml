...
ColumnLayout {
    ...
    readonly property real currentVolume: sliderHandler.active ? dragVolume : Audio.speakerVolume

    // Mouse wheel scrolling
    WheelHandler {
        onWheel: (event) => {
            let step = event.angleDelta.y > 0 ? 5 : -5;
            Audio.setSpeakerVolume(Audio.speakerVolume + step);
        }
    }
    ...
    Rectangle {
        ...
        Rectangle {
            ...
            height: (parent.height / 100) * (Audio.speakerMuted ? 0 : root.currentVolume)
    ...
    Rectangle {
        ...
        Text {
            ...
            text: {
                if (Audio.speakerMuted || root.currentVolume === 0) return "🕨";
                if (root.currentVolume < 50) return "🕩";
                return "🕪";
            }
        ...
        TapHandler { onTapped: Audio.toggleSpeakerMute() }
